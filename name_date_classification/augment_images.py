...
for root, dirs, files in os.walk(image_directory):
    for filename in files:
        if filename.startswith('rot_') and filename.endswith('.png'):  # Only process rotated images
            image_path = os.path.join(root, filename)
...
print("Data Augmentation: Rotation and color jitter augmentation completed successfully!")
